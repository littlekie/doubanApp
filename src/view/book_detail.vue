		...
		<div class="movie_box" v-else>
			...
			<div class="detail_top">
				<div class="detail_grade">
					...
					<div class="star">
						<i class="iconfont icon-star on" v-for='star in onStar(detail.rating.average)'></i>
		  				<i class="iconfont icon-star"  v-for='star in offStar(detail.rating.average)'></i>
		  			</div>
		  			<p>{{comments.total}}人</p>
				</div>
				<div class="detail_title">
					<h2>{{detail.title}}</h2>
					<div class="detail_txt">
						<p>{{detail.author[0]}} / {{detail.price}}</p>
						<p>出版社: {{detail.publisher}}(中国大陆)</p>
					</div>
				</div>
			</div>

			 <div class="detai_content">
				<h3 class="detail_h">书籍简介</h3>
				<p class="detail_desc">{{summary}}<a href="javascript:void(0)" class="open_all" v-if="is_all" @click="openAll">展开</a></p>
			</div>
			<div class="detai_content">
				<h3 class="detail_h">作者</h3>
			...
			<div id="scroll_tab" class="tab_box">
				<ul>
			  		<li :class="{active:is_comment}" @click="is_comment=true">短论</li>
			  		<li :class="{active:!is_comment}" @click="is_comment=false">长评</li>
			  	</ul>
			</div>
			...
			<div v-show='!is_comment'>
				...
				<router-link :to="{name:'discuss',params:{id:this.$route.params.id}}" class="more_comment" href="javascript:void(0)">全部长评{{discuss.total}}条</router-link>
			</div>
...
<script>
	...
	export default{
        ...
        this.$http.jsonp(Api.url+'/v2/book/'+thisId).then(function(response){
        ...
        this.$http.jsonp(Api.url+'/v2/book/'+bookId+'/reviews?apikey=0b2bdeda43b5688921839c8ecb20399b&start=0&count=5').then(function(response){
          self.discuss=response.body;
			...
			getComment(){
				var self=this;
				var bookId=this.$route.params.id;
        this.$http.jsonp(Api.url+'/v2/book/'+bookId+'/comments?apikey=0b2bdeda43b5688921839c8ecb20399b&start=0&count=5').then(function(response){
